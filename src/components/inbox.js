...
const Inbox = () => {
    ...
    return (
        ...
        className='inboxblack'>

        <div style={{display: 'grid', gridTemplateColumns: '100%', gridTemplateRows: '25px', overflow: 'hidden'}}>
            <div className='top-bar' style={{width: '100%', height: '25px', display: 'flex', backgroundColor: 'darkblue', alignItems: 'center', justifyContent: 'space-between'}}>
                <div style={{display: 'flex', alignItems: 'center'}}>
                    ...
                    <h4 className='title' style={{marginLeft: '2px'}}>Inbox</h4>
                </div>
                <div className='buttons content' style={{marginRight: '1px'}}>
                    <button className='top-button X' onClick={() => onClick()}><p style={{margin: 0,  padding: '1px 3px'}}>X</p></button>
                    <button className='top-button m'><p style={{margin: 0,  padding: '1px 3px'}}>◻</p></button>
                    <button className='top-button m' onClick={() => onClick()}><p style={{margin: 0,  padding: '1px 3px'}}>_</p></button>
                </div>
            </div>
        </div>
        <div className='file-bar content' onMouseLeave={() => hideDrops()} 
        style={{width: '100%', height: '25px', display: 'flex', alignItems: 'flex-start', gridRow: '2', cursor: "url(/images/cursor/arrow.cur), auto", border: 'groove 2px'}}>
            <button className='files' style={{gridTemplateColumns: '100%', padding: '0'}}><p style={{margin: 0,  padding: '1px 3px'}} onClick={() => showDrops()}>File</p>
            ...
            <button className='edits' style={{gridTemplateColumns: '100%', padding: '0'}}><p style={{margin: 0,  padding: '1px 3px'}} onClick={() => showDrops()}>Edit</p>
            <div className='hidemenu dropmenu'>
                    <p className='edit'>Copy Link</p>
                </div>
            </button>
            <button className='views' style={{gridTemplateColumns: '100%', padding: '0'}}><p style={{margin: 0,  padding: '1px 3px'}} onClick={() => showDrops()}>View</p>
            <div className='hidemenu dropmenu'>
                    <p className='view'>Zoom</p>
                    <p className='view'>Find</p>
                </div>
            </button>
            <button className='favorites' style={{gridTemplateColumns: '100%', padding: '0'}}><p style={{margin: 0,  padding: '1px 3px'}} onClick={() => showDrops()}>Mail</p>
            <div className='hidemenu dropmenu'>
            ...
            <button className='helps' style={{gridTemplateColumns: '100%', padding: '0'}}><p style={{margin: 0,  padding: '1px 3px'}} onClick={() => showDrops()}>Help</p>
            <div className='hidemenu dropmenu'>
...
<div style={{display: 'grid', gridTemplateColumns: '19% 1% 80%', gridTemplateRows: '30% 70%', height: '-moz-calc(100% - 105px)', height: '-webkit-calc(100% - 105px)', height: 'calc(100% - 105px)'}}>
        <div style={{gridRow: 'span 2', gridColumn: 1, border: 'inset 3px', overflow: 'hidden', cursor: "url(/images/cursor/arrow.cur), auto", backgroundColor: 'white'}}>
            <div style={{display: 'flex', alignItems: 'center', marginLeft: '5px'}}><img alt='decor' width={'16px'} src='/images/Mail.ico'/><p style={{ margin: 0}}>Microsoft Exchange</p></div>
            <div style={{marginLeft: '35px'}}>
                ...
                <li style={{display: 'flex'}}><img alt='decor' width={'16px'} src='/images/winicon/inbox/204.ico' /><p style={{ margin: '0 5px'}}>Outbox</p></li>
                <li style={{display: 'flex'}}><img alt='decor' width={'16px'} src='/images/winicon/inbox/203.ico' /><p style={{ margin: '0 5px'}}>Sent</p></li>
                <li style={{display: 'flex'}}><img alt='decor' width={'16px'} src='/images/winicon/inbox/203.ico' /><p style={{ margin: '0 5px'}}>Deleted</p></li>
                <li style={{display: 'flex'}}><img alt='decor' width={'16px'} src='/images/winicon/inbox/203.ico' /><p style={{ margin: '0 5px'}}>Drafts</p></li>
            </div>
        </div>
        <div style={{gridRow: 1, gridColumn: 3, border: 'inset 3px', overflow: 'hidden', cursor: "url(/images/cursor/arrow.cur), auto"}}>
            <div className='mailbox' style={{height: '100%', overflowY: 'scroll', overflowX: 'scroll', display: 'flex', flexDirection: 'column', backgroundColor: 'white'}}>
                ...
                {state.email.map((num) => {
                    ...
                    return (
                        <div className='maillist' id={`mail-${email.id}`} onClick={(e) => openMail(e.currentTarget.id)} style={{width: '100%', display: 'grid', gridTemplateColumns: 'calc(5% + 25px) 15% 15% 65%', gap: '25px', gridTemplateRows: '100%', zIndex: '1', cursor: "url(/images/cursor/pointer.cur), auto"}} key={email.id}>
                            ...
                            <p style={{ margin: 0, whiteSpace: 'nowrap', overflow: 'hidden'}}>{email.from}</p>
                            <p style={{ margin: 0, whiteSpace: 'nowrap', overflow: 'hidden'}}>{email.title}</p>
                            <p style={{ margin: 0, whiteSpace: 'nowrap', overflow: 'hidden'}}>{email.text}</p>
                        </div>
                    )
                })}
            </div>
        </div>
        <div className='openmail' style={{marginTop: '5px', gridRow: 2, gridColumn: 3, border: 'inset 3px', overflow: 'hidden', cursor: "url(/images/cursor/arrow.cur), auto", backgroundColor: 'white', padding: '10px'}}>
            <p>From: {open.from}</p>
            <h3>{open.title}</h3>
            <p>{open.text}</p>
        </div>
